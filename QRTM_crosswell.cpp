...
)
{
	...
	if(flag==1)
	{
		...
		fp=fopen("./output/output_crosswell/ricker.dat","wb");    
		for(it=0;it<nt;it++)
		{
	...
	if(flag==2)
	{
		...
		fp=fopen("./output/output_crosswell/ricker_integration.dat","wb");    
		for(it=0;it<nt;it++)
		{
			fwrite(&ricker[it],sizeof(float),1,fp);
		}    
	...
	{	
		...
		fp=fopen("./output/output_crosswell/ricker_derivative.dat","wb");    
		for(it=0;it<nt;it++)
		{
			fwrite(&ricker[it],sizeof(float),1,fp);
		}    
...
)
{
	int ip,ipp,iz,ix; 
	FILE *fp;
	fp=fopen("./input/para_crosswell/acc_vp.dat","rb");
	...
	fp=fopen("./input/para_crosswell/acc_Qp.dat","rb");
	for(ix=L;ix<ntx-L;ix++)
	{
		for(iz=L;iz<ntz-L;iz++)
		{
