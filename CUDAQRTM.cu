...
)
{
	...
	for(i=0;i<GPU_N;i++)
	{
		...
		gpuErrchk(cudaMallocHost((void **)&plan[i].u0, sizeof(cufftComplex)*ntp));		
		gpuErrchk(cudaMallocHost((void **)&plan[i].u1, sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMallocHost((void **)&plan[i].u2, sizeof(cufftComplex)*ntp));		

		gpuErrchk(cudaMallocHost((void **)&plan[i].seismogram_obs, sizeof(float)*nt*rnmax));
		gpuErrchk(cudaMallocHost((void **)&plan[i].seismogram_dir, sizeof(float)*nt*rnmax));
		gpuErrchk(cudaMallocHost((void **)&plan[i].seismogram_syn, sizeof(float)*nt*rnmax));
		gpuErrchk(cudaMallocHost((void **)&plan[i].seismogram_rms, sizeof(float)*nt*rnmax));
		
		gpuErrchk(cudaMallocHost((void **)&plan[i].image_sources, sizeof(float)*ntp));
		gpuErrchk(cudaMallocHost((void **)&plan[i].image_receivers, sizeof(float)*ntp));
		gpuErrchk(cudaMallocHost((void **)&plan[i].image_cor, sizeof(float)*ntp));
		gpuErrchk(cudaMallocHost((void **)&plan[i].image_nor, sizeof(float)*ntp));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_r_ix,sizeof(int)*rnmax));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_r_iz,sizeof(int)*rnmax));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_ricker,sizeof(float)*nt));        //ricker

		gpuErrchk(cudaMalloc((void**)&plan[i].d_vp,size_model));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_Gamma,size_model));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_u0,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u1,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u2,sizeof(cufftComplex)*ntp));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_u0_inv,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u1_inv,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u2_inv,sizeof(cufftComplex)*ntp));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_t_cp,sizeof(int)*N_cp));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u_cp,size_model*N_cp));		//checkpoints

		gpuErrchk(cudaMalloc((void**)&plan[i].d_kx,sizeof(float)*ntx));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_kz,sizeof(float)*ntz));

		gpuErrchk(cudaMalloc((void **)&plan[i].d_uk,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_uk0,sizeof(cufftComplex)*ntp));

		gpuErrchk(cudaMalloc((void **)&plan[i].d_uk_inv,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_uk0_inv,sizeof(cufftComplex)*ntp));

		gpuErrchk(cudaMalloc((void **)&plan[i].d_Lap_uk,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_amp_uk,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_pha_uk,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_sta_uk,sizeof(cufftComplex)*ntp));

		gpuErrchk(cudaMalloc((void **)&plan[i].d_Lap,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_amp_Lap,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_pha_Lap,sizeof(cufftComplex)*ntp));
		gpuErrchk(cudaMalloc((void **)&plan[i].d_sta_Lap,sizeof(cufftComplex)*ntp));
							
		gpuErrchk(cudaMalloc((void**)&plan[i].d_seismogram,sizeof(float)*nt*rnmax));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_seismogram_rms,sizeof(float)*nt*rnmax));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_borders_up,sizeof(float)*nt*nx));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_borders_bottom,sizeof(float)*nt*nx));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_borders_left,sizeof(float)*nt*nz));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_borders_right,sizeof(float)*nt*nz));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_u2_final0,size_model));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_u2_final1,size_model));

		gpuErrchk(cudaMalloc((void**)&plan[i].d_image_sources,size_model));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_image_receivers,size_model));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_image_cor,size_model));
		gpuErrchk(cudaMalloc((void**)&plan[i].d_image_nor,size_model));
	}
}
